...
// We base this on your subnet instead of simply on your IP. Useful for large server communities.
local explodedIP = {"0", "0", "0", "0"}

http.Get("http://automation.whatismyip.com/n09230945.asp", "", function(ip)
		explodedIP = string.Explode(".", ip)
        print("Retrieved server's IP-address: " .. ip);
end);

function GM:GetServerIP( )

	return string.Implode( "." , explodedIP )
	...
	http.Get("http://steamcommunity.com/profiles/7656" .. communityid .. "/?xml=1", "", function(c, s)		
		...
		if(xml[2][1][1] ~= "0" and xml[2][1][1] ~= "<![CDATA[The specified profile could not be found.]]>") then
			if(xml[2][5][1] == "public") then
				...
				kickPlayer(ply, "Make your community profile public.")
			end
		else
...
hook.Add("PlayerInitialSpawn", "Anti-Serenity", function(ply)
	timer.Simple(0.1, checkPlayer, ply);
end);*/
